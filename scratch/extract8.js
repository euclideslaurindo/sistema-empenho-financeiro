...
const logPath = path.join(basePath, 'scratch/log.txt');
...
try {
  ...
  log('empenhoStart: ' + empenhoStart);
  log('reciboStart: ' + reciboStart);
  log('mainPageStart: ' + mainPageStart);

  if (empenhoStart === -1 || reciboStart === -1 || mainPageStart === -1) {
    log('Could not find component boundaries');
    process.exit(1);
  }
